...
def get_sign(cells_info_file): #解析细胞信息
    ...
    nuclei_area_abnormal = find_abnormal_data(nuclei_area_, 0.2) #单项阈值
    nuclei_rule_abnormal = find_abnormal_data(nuclei_rule_, 0.2)
    nuclei_value_abnormal = find_abnormal_data(nuclei_value_, 0.2)
    cytoplasm_area_abnormal = find_abnormal_data(cytoplasm_area_, 0.2)
    cytoplasm_rule_abnormal = find_abnormal_data(cytoplasm_rule_, 0.2)
    cytoplasm_value_abnormal = find_abnormal_data(cytoplasm_value_, 0.2)
    cell_N_C_abnormal = find_abnormal_data(cell_N_C_, 0.2)
    ...
    cells_infos_2 = cells_infos
    cells_infos_2[0,:] = cells_infos[0,:] * 0.2 #综合权重
    cells_infos_2[1,:] = cells_infos[1,:] * 0.05
    cells_infos_2[2,:] = cells_infos[2,:] * 0.2
    cells_infos_2[3,:] = cells_infos[3,:] * 0.05
    cells_infos_2[4,:] = cells_infos[4,:] * 0.05
    cells_infos_2[5,:] = cells_infos[5,:] * 0.1
    cells_infos_2[6,:] = cells_infos[6,:] * 0.35
    cells_infos_2 = np.array(cells_infos_2)
    cnt_2 = 0
    for n in range(0, cells_infos_2.shape[1]):
        sum_ = cells_infos_2[:,n].sum()
        if sum_ >= 0:
