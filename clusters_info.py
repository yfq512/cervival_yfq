...
def main2():
    ...
    for n,i in zip(listcells,tqdm(range(len(listcells)))):
     ...
     #   cv2.imwrite(cellpath+'abc0.png',img_gray)
        ...
        cell_nuclei_mask1,area1 = get_cell_nuclei_mask(img_gray, img, value_1) #获取细胞核掩码、个数、面积、凸面积、周长、核形规则度、获取细胞>核深染程度
        cell_nuclei_mask2,area2 = get_cell_nuclei_mask(img_gray, img, value1_) #获取细胞核掩码、个数、面积、凸面积、周长、核形规则度、获取细胞>核深染
...
if __name__ == "__main__":
    ...
    for n,i in zip(listcells,tqdm(range(len(listcells)))):
     ...
     #   cv2.imwrite(cellpath+'abc0.png',img_gray)
        value1_,value_2 = bf.get_2value(img_gray)
        value_1,_ = bf.get_2value(img_gray, grien = 3)
        cell_nuclei_mask1,area1 = get_cell_nuclei_mask(img_gray, img, value_1) #获取细胞核掩码、个数、面积、凸面积、周长、核形规则度、获取细胞核深染程度
        cell_nuclei_mask2,area2 = get_cell_nuclei_mask(img_gray, img, value1_) #获取细胞核掩码、个数、面积、凸面积、周长、核形规则度、获取细胞核深染
    #    cv2.imwrite(cellpath+'abc1.png',cell_nuclei_mask2)
        temp = area1/area2
        temp1 = str(temp)[0:5]
        if area1/area2 > 0.85:
