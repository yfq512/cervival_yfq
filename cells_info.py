...
if __name__ == "__main__":
    ...
    for n in listcells:
        ...
        sign_nuclei, cell_nuclei_mask, nuclei_cnt, nuclei_area, nuclei_circ, nuclei_rule, cell_nuclei_value = get_cell_nuclei_mask(img_gray, img, value_1) #获取细胞核掩码、个数、面积、周长、核形规则度、获取细胞核深染程度
        if sign_nuclei == 0:
            continue
        sign_cytoplasm, cell_cytoplasm_mask, cytoplasm_area, cytoplasm_circ, cytoplasm_rule, cell_cytoplasm_value = get_cell_cytoplasm_mask(img_gray, cell_nuclei_mask, img, value_2) #获取细胞质掩码、面积、周长、细胞规则度、获取细胞质情况
        if sign_cytoplasm == 0:
            continue
